...
const createTask = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      ...
      task: { ...task._doc, completedTodoCount: completedCount },
    });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
...
const updateTask = async (req, res) => {
  try {
    ...
    const completedCount = updatedTask.todoChecklist.filter(
      (item) => item.completed
    ).length;

    res.json({
      message: "Task updated successfully",
      updateTask: { ...updatedTask._doc, completedTodoCount: completedCount },
    });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
...
const updateTaskChecklist = async (req, res) => {
  try {
    ...
    res.json({
      ...
      task: { ...updatedTask._doc, completedTodoCount: completedCount },
    });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
